...
const PLUGIN_ID = "auto-translator";
...
const useQueryParams = (initialParams) => {
  ...
  const query = React.useMemo(() => {
    ...
    return {
      ...initialParams,
      ...lib.parse(searchQuery)
    };
  }, [
    search,
    initialParams
  ]);
  const setQuery = React.useCallback((nextParams, method = "push", replace2 = false) => {
    let nextQuery = {
      ...query
    };
    if (method === "remove") {
    ...
    navigate({
      search: lib.stringify(nextQuery, {
        encode: false
      })
    }, {
      replace: replace2
...
const TranslateButton = ({
  ...
  const [showLocaleSelector, setShowLocaleSelector] = React.useState(false);
  const [translationProgress, setTranslationProgress] = React.useState("");
  const fetchLocales = async () => {
    try {
      const { data } = await get2("/auto-translator/locales");
      const availableLocales = data.data.filter(
        (locale) => locale.code !== currentLocale && locale.code !== "en" && !locale.isDefault
      );
    ...
    try {
      if (selectedLocale === "all") {
        ...
        for (let i2 = 0; i2 < locales.length; i2++) {
          const locale = locales[i2];
          setTranslationProgress(
            `Translating to ${locale.name} (${i2 + 1}/${locales.length})...`
          );
          try {
            await post("/auto-translator/translate", {
              data: {
                contentType,
                documentId: isSingleType ? void 0 : documentId,
      ...
      const errorMessage = error?.response?.data?.error?.message || error?.message || formatMessage({ id: getTrad("message.error") });
      toggleNotification({
        type: "danger",
        message: errorMessage
      });
  ...
  if (!showLocaleSelector) {
    return /* @__PURE__ */ jsxRuntime.jsx(
      designSystem.Button,
      {
        startIcon: /* @__PURE__ */ jsxRuntime.jsx(icons.Globe, {}),
        onClick: fetchLocales,
        variant: "secondary",
        size: "S",
        style: { width: "100%" },
        children: formatMessage({ id: getTrad("button.translate") })
      }
    );
  }
      ...
      children: [
        /* @__PURE__ */ jsxRuntime.jsxs(
          designSystem.Combobox,
          {
            ...
            onChange: (value) => setSelectedLocale(String(value)),
            size: "S",
            style: { width: "100%" },
            children: [
              /* @__PURE__ */ jsxRuntime.jsxs(designSystem.ComboboxOption, { value: "all", children: [
                "🌐 All locales (",
                locales.length,
                ")"
              ] }, "all"),
              locales.map((locale) => /* @__PURE__ */ jsxRuntime.jsx(designSystem.ComboboxOption, { value: locale.code, children: locale.name }, locale.code))
            ]
          }
        ),
        translationProgress && /* @__PURE__ */ jsxRuntime.jsx("div", { style: { fontSize: "12px", color: "#666", width: "100%" }, children: translationProgress }),
        /* @__PURE__ */ jsxRuntime.jsxs(
          "div",
          {
            style: {
            ...
            children: [
              /* @__PURE__ */ jsxRuntime.jsx(
                designSystem.Button,
                {
                  ...
                  size: "S",
                  style: { width: "100%" },
                  children: isLoading ? formatMessage({ id: getTrad("button.translating") }) : formatMessage({ id: getTrad("button.translate") })
                }
              ),
              /* @__PURE__ */ jsxRuntime.jsx(
                designSystem.Button,
...
const InjectedTranslateButton = () => {
  const { get: get2 } = useFetchClient();
  const [{ query }] = useQueryParams();
  const [isI18nEnabled, setIsI18nEnabled] = React.useState(false);
  ...
  const shouldRender = isI18nEnabled && currentLocale && (isSingleType || documentId);
  if (!shouldRender) {
    return null;
  }
  return /* @__PURE__ */ jsxRuntime.jsx(
